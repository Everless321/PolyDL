  ...
  private async downloadFile(
    ...
    try {
      ...
      try {
        ...
        if (len) {
          const n = Number(len)
          if (Number.isFinite(n) && n > 0) expectedSize = n
        }
      } catch {
        /* HEAD 不支持或失败时静默降级，下面用 response 事件兜底 */
      }
      ...
      // 完整性校验：已知 Content-Length 时严格比对，抛错交由外层 catch 清理并返回失败
      if (expectedSize !== null && tmpStat.size !== expectedSize) {
        throw new Error(
          `Incomplete download: expected ${expectedSize} bytes, got ${tmpStat.size} bytes`
        )
      }
